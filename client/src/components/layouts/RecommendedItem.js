...
const RecommendedItem = ({ title, year, category, rating, thumbnail }) => {
  // Declare local state
  ...
  return (
    <div className='recommended-item'>
      <div
        className='recommended-img-cont'
        style={{
          background: !isHovering
            ? `url(${thumbnail.regular.small})`
            : `linear-gradient(0deg, rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url(${thumbnail.regular.small})`,
        }}
      ...
      <div className='recommended-info'>
        <div className='recommended-top'>
          ...
          <div className='grey-dot'></div>
          <p className='body-s'>{rating}</p>
        </div>
        <h4 className='heading-xs'>{title}</h4>
      </div>
    </div>
  );
};
